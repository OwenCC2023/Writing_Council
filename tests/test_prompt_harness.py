...
def test_idea_is_not_asked_when_a_source_story_is_given(capsys, tmp_path):
    story = tmp_path / "s.txt"
    story.write_text("prose", encoding="utf-8")
    answers = {
        "story title": "T",
    ...
    # stdout is where an idea question would show up.
    assert "Story idea" not in capsys.readouterr().out
